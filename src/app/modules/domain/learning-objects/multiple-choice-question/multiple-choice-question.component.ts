...
export class MultipleChoiceQuestionComponent implements OnInit, LearningObjectComponent {
  learningObject: MultipleChoiceQuestion;
  checked: string;
  evaluation: McqEvaluation;

  constructor(private mcqService: MultipleChoiceQuestionService,
  ...
  onSubmit(): void {
    this.mcqService.submit(this.learningObject.id, this.checked)
